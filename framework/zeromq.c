...
void zeromq_thread() {
	...
	
	{
		...
		if (zmq_socket_monitor(objectSocket, "inproc://connectfour-zeromq-monitor", ZMQ_EVENT_ALL) == -1) {
			printf("zeromq: %s\n", zmq_strerror(zmq_errno()));
		}
		
		if (zmq_connect(objectMonitor, "inproc://connectfour-zeromq-monitor") == -1) {
			printf("zeromq: %s\n", zmq_strerror(zmq_errno()));
		}
...
void zeromq_start() {
	...
	pthread_t objectThread = 0; pthread_create(&objectThread, NULL, (void*) (zeromq_thread), NULL);
}
